...
const outputZip = path.join(releaseDir, `mej-progress-delta-v${version}.zip`);
const archiveItems = [
  "module.json",
  "scripts",
  "styles",
...
execSync(
  `cd ${shellQuote(root)} && zip -r ${shellQuote(outputZip)} ${archiveItems.map(shellQuote).join(" ")} -x '*.DS_Store'`,
  { stdio: "inherit" }
);
